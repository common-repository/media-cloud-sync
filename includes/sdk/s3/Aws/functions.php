...
use Dudlewebs\WPMCS\s3\GuzzleHttp\Client;
use Dudlewebs\WPMCS\s3\Psr\Http\Message\RequestInterface;
use Dudlewebs\WPMCS\s3\GuzzleHttp\ClientInterface;
...
function default_http_handler()
{
    ...
    if ($version === 6 || $version === 7) {
        return new \Dudlewebs\WPMCS\s3\Aws\Handler\GuzzleV6\GuzzleHandler();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return new \Dudlewebs\WPMCS\s3\Aws\Handler\GuzzleV5\GuzzleHandler();
    }
...
function default_user_agent()
{
    ...
    // If Guzzle 6 or 7 installed
    if ($version === 6 || $version === 7) {
        return \Dudlewebs\WPMCS\s3\GuzzleHttp\default_user_agent();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return \Dudlewebs\WPMCS\s3\GuzzleHttp\Client::getDefaultUserAgent();
    }
    throw new \RuntimeException('Unknown Guzzle version: ' . $version);
}
/**
...
function guzzle_major_version()
{
    ...
    if (\defined('Dudlewebs\\WPMCS\\s3\\GuzzleHttp\\ClientInterface::VERSION')) {
    ...
    } elseif (\defined('Dudlewebs\\WPMCS\\s3\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
