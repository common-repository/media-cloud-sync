...
use Dudlewebs\WPMCS\s3\Aws\Api\Validator;
use Dudlewebs\WPMCS\s3\Aws\Api\ApiProvider;
use Dudlewebs\WPMCS\s3\Aws\Api\Service;
use Dudlewebs\WPMCS\s3\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use Dudlewebs\WPMCS\s3\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use Dudlewebs\WPMCS\s3\Aws\ClientSideMonitoring\Configuration;
...
use Dudlewebs\WPMCS\s3\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use Dudlewebs\WPMCS\s3\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use Dudlewebs\WPMCS\s3\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use Dudlewebs\WPMCS\s3\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use Dudlewebs\WPMCS\s3\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use Dudlewebs\WPMCS\s3\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use Dudlewebs\WPMCS\s3\Aws\EndpointDiscovery\ConfigurationInterface;
use Dudlewebs\WPMCS\s3\Aws\EndpointDiscovery\ConfigurationProvider;
use Dudlewebs\WPMCS\s3\Aws\Exception\InvalidRegionException;
use Dudlewebs\WPMCS\s3\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use Dudlewebs\WPMCS\s3\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use Dudlewebs\WPMCS\s3\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use Dudlewebs\WPMCS\s3\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
...
use Dudlewebs\WPMCS\s3\Aws\Credentials\CredentialProvider;
...
class ClientResolver
{
    ...
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            ...
            throw new IAE('Credentials must be an instance of ' . 'Dudlewebs\\WPMCS\\s3\\Aws\\Credentials\\CredentialsInterface, an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    ...
    {
        if ($value === \false) {
            $value = new Configuration(\false, \Dudlewebs\WPMCS\s3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \Dudlewebs\WPMCS\s3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \Dudlewebs\WPMCS\s3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
        $list->appendBuild(ApiCallMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallMonitoringMiddleware');
        $list->appendAttempt(ApiCallAttemptMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallAttemptMonitoringMiddleware');
    }
    ...
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            if (!self::isValidRegion($args['region'])) {
                throw new InvalidRegionException('Region must be a valid RFC' . ' host label.');
            }
            ...
            $args['region'] = \Dudlewebs\WPMCS\s3\Aws\strip_fips_pseudo_regions($args['region']);
            // Invoke the endpoint provider and throw if it does not resolve.
            $result = EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme'], 'options' => self::getEndpointProviderOptions($args)]);
    ...
    public static function _apply_use_dual_stack_endpoint($value, array &$args)
    {
        ...
        if ($value instanceof UseDualStackEndpointConfigurationInterface) {
            ...
            // The Configuration class itself will validate other inputs
            $args['config']['use_dual_stack_endpoint'] = new UseDualStackEndpointConfiguration($value, $args['region']);
        }
    }
    ...
    {
        if ($value !== \false) {
            $list->interpose(new TraceMiddleware($value === \true ? [] : $value, $args['api']));
        }
    }
    ...
    public static function _apply_validate($value, array &$args, HandlerList $list)
    {
        ...
        $validator = $value === \true ? new Validator() : new Validator($value);
    ...
    {
        return new WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, HandlerList $list)
    {
        $args['handler'] = new WrappedHttpHandler($value, $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    ...
    public static function _apply_idempotency_auto_fill($value, array &$args, HandlerList $list)
    {
        ...
        if ($enabled) {
            $list->prependInit(IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    ...
    {
        return Service::createSerializer($args['api'], $args['endpoint']);
    }
