...
use Dudlewebs\WPMCS\s3\GuzzleHttp\HandlerStack;
...
class MockHandler implements \Countable
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        $response = $response instanceof \Exception ? \Dudlewebs\WPMCS\s3\GuzzleHttp\Promise\rejection_for($response) : \Dudlewebs\WPMCS\s3\GuzzleHttp\Promise\promise_for($response);
        return $response->then(function ($value) use($request, $options) {
            ...
            if (isset($options['sink'])) {
                ...
                } elseif ($sink instanceof \Dudlewebs\WPMCS\s3\Psr\Http\Message\StreamInterface) {
                    $sink->write($contents);
                }
            }
            return $value;
    ...
    public function append()
    {
        foreach (\func_get_args() as $value) {
            if ($value instanceof ResponseInterface || $value instanceof \Exception || $value instanceof PromiseInterface || \is_callable($value)) {
                ...
                throw new \InvalidArgumentException('Expected a response or ' . 'exception. Found ' . \Dudlewebs\WPMCS\s3\GuzzleHttp\describe_type($value));
            }
        }
    }
    /**
