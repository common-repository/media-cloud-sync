...
class FilterContent {
	...
	public function filter_post_backward( $content ) {
		...
		add_filter( 'wpmcs_enable_backward_url_repalcement', [ $this, 'enable_backward_url_check' ] );

		$content = $this->filter_post( $content );

		// remove backword compatibility after process
		remove_filter( 'wpmcs_enable_backward_url_repalcement', [ $this, 'enable_backward_url_check' ] );
	...
	protected function pre_filter_content( $content ) {
		...
		return $wpmcsService->remove_query_strings( $content, $base_url );
	}
	...
	protected function get_urls_from_img_src( $content, &$to_cache ) {
		...
		foreach ( $matches as $image ) {
			...
			$item_sources[ $bare_url ] = absint( $class_id[1] );
		}
		...
		if ( count( $item_sources ) > 1 ) {
			 ...
			 */
			update_meta_cache( 'post', array_unique( $item_sources ) );
		}
	...
	public function url_needs_replacing( $url ) {
		$reverse_compatibility = apply_filters('wpmcs_enable_backward_url_repalcement', false);
	...
	public function item_matches_src( $item_source, $url ): bool {
		...
		$item 		= $wpmcsItem->get( $item_source );


		$backward_replacement = apply_filters('wpmcs_enable_backward_url_repalcement', false);

		if (
			Utils::is_empty($item) ||
			get_post_type( $item_source ) !== 'attachment'
		) {
			return false;
		}
		...
		$bare_url[] = $backward_replacement 
							? Utils::reduce_url($wpmcsItem->get_url( $item_source )) 
							: Utils::reduce_url(trailingslashit($upload_dir['baseurl']).$item['source_path']);	
		
	
		$extras = $wpmcsItem->get_extras( $item_source );

		if(isset($extras['original']) && !Utils::is_empty($extras['original'])) {
			$bare_url[] = $backward_replacement 
							? Utils::reduce_url($wpmcsItem->get_url( $item_source, 'original' ))
							: Utils::reduce_url(trailingslashit($upload_dir['baseurl']).$extras['original']['source_path']);
		}
		...
		if(isset($extras['sizes']) && !Utils::is_empty($extras['sizes'])) {
			foreach($extras['sizes'] as $size => $sub_image) {
				if(!Utils::is_empty($sub_image)){
					$bare_url[] = $backward_replacement 
									? Utils::reduce_url($wpmcsItem->get_url( $item_source, $size ))
	...
	protected function push_to_url_pairs( &$url_pairs, $item_source, $find, &$to_cache ) {
		...
		$source_cloud_file				= Utils::remove_scheme($wpmcsItem->get_url($item_source));
		...
		if(isset($extras['original']) && !Utils::is_empty($extras['original'])) {
			$original_server_file 				= Utils::remove_scheme($base.$extras['original']['source_path']);
			$original_cloud_file 				= Utils::remove_scheme($wpmcsItem->get_url($item_source, 'original'));
		...
		if(isset($extras['sizes'])){
			foreach ($extras['sizes'] as $size => $sub_image) {
				...
				$sub_cloud_file 				= Utils::remove_scheme($wpmcsItem->get_url( $item_source, $size ));
	...
	protected function get_item_sources_from_urls( $urls ) {
		...
		foreach ( $query_set as $url ) {
			...
			$path = Utils::get_attachment_source_path( $full_url );
		...
		if ( ! empty( $paths ) ) {
			$wpmcsItems = $wpmcsItem->get_items_by_source_paths( array_keys( $paths ) );
			...
			if ( ! empty( $wpmcsItems ) ) {
				foreach ( $wpmcsItems as $item ) {
					...
					$original 		= $wpmcsItem->get_extras($source_id, 'original');
					if(isset($original['source_path'])) {
						$source_paths[] = $original['source_path'];
					}
